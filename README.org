...
After installing, use the library with the compilation flag src_cpp{'val-config --cflags'} (for finding the headers)
and the linking flag src_cpp{'val-config --libs'} (to link against the library).
