...
After installing, use the library with the compilation flag src_makefile{'val-config --cflags'} (for finding the headers)
and the linking flag src_makefile{'val-config --libs'} (to link against the library).
